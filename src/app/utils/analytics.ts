...
export function filterByDateRange<
  T extends { bill_date?: string | Date; tran_date?: string | Date }
>(items: T[], range: DateRange): T[] {
...
export function calculateAnalytics(
  ...
  // Calculate totals from bills (money earned) - using paid_sum
  const totalEarned = filteredBills.reduce((sum, bill) => {
    return sum + (bill.paid_sum || 0);
  }, 0);
  ...
  // Top buying customers (from bills) - using paid_sum
  const customerSpending = new Map<
    string,
    { spent: number; transactions: number }
  >();

  filteredBills.forEach((bill) => {
    const custId = bill.cust_id;
    const amount = bill.paid_sum || 0;

    const current = customerSpending.get(custId) || {
      spent: 0,
      transactions: 0,
    };
  ...
  filteredBills.forEach((bill) => {
    ...
    if (date) {
      ...
      monthlyData.set(month, {
        ...current,
        earned: current.earned + (bill.paid_sum || 0),
      });
    }
  });
